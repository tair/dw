 ...
 */
public abstract class AbstractGeneDelegateTest {
  /** Define a class logger. */
  protected static Logger logger = Logger.getLogger(AbstractGeneDelegateTest.class);
  ...
  protected GeneDelegate delegate = null;
  ...
  @Before
  public void setUp() {
    delegate = GenomicregionDelegateFactory.getGeneDelegate();
  ...
  protected void clearCaches() {
    // Primary class Gene cache
    IDaoManager manager = DaoManagerFactory.getManager(getSubsystem());
    if (manager != null) {
      manager.clearCache(org.tair.db.genomicregion.Gene.class.getName());
    }
  }
   ...
   * @throws DelegateException when a problem occurs during object creation
   */
  protected List<org.tair.bs.genomicregion.BsGene> createGenomicregionGene(int count) 
      throws DelegateException, InvalidParametersException {
    List<org.tair.bs.genomicregion.BsGene> objects = 
      new CopyOnWriteArrayList<org.tair.bs.genomicregion.BsGene>();
    ...
    for (int i = 0; i < count; i++) {
      ...
      java.lang.String curatedSummary = 
        com.poesys.cartridges.db.utilities.StringUtilities.generateString();

      // Create the object.
      BsGene object = null;
      try {
        object = delegate.createGene(tairObjectId, name, dateEntered, parentRegionTairObjectId, genome, curatedSummary);
  ...
  public void testInsert() {
    // Create a new Gene object to perform the test.
    List<BsGene> objects = createGenomicregionGene(1);
    ...
    BsGene queriedObject = queryStoredObject(key);
   ...
   */
  protected BsGene queryStoredObject(com.poesys.db.pk.NaturalPrimaryKey key)
      throws DelegateException {
    // Clear the caches, then query the saved object and test it.
    clearCaches();

    BsGene queriedObject = delegate.getObject(key);
   ...
   *
   * @param object the BsGene object to update
   */
  protected void updateColumn(BsGene object) {
    try {
      object.setCuratedSummary("Gene.curatedSummary updated");
   ...
   * @see #updateColumn
   */
  protected boolean isColumnUpdated(BsGene object) {
    boolean retVal = false;
    retVal = object.getCuratedSummary().equals("Gene.curatedSummary updated");
    return retVal;
  }
   ...
   *
   * @param original the original, updated BsGene object
   * @param queried the queried BsGene object
   * @return true if the column is updated, false if not
   */
  protected boolean isColumnUpdated(BsGene original, BsGene queried) {
    boolean retVal = true;
    retVal = 
      retVal && queried.getCuratedSummary().equals(original.getCuratedSummary());
    return retVal;
  }
  ...
  public void testUpdateBatch() throws InvalidParametersException,
      DelegateException {
    List<BsGene> objects = createGenomicregionGene(2);
    delegate.insert(objects);
    // Allocate a map to hold the updated objects for later comparison.
    java.util.Map<com.poesys.db.pk.IPrimaryKey, BsGene> map = 
      new java.util.TreeMap<com.poesys.db.pk.IPrimaryKey, BsGene>();
    for (BsGene object : objects) {
      updateColumn(object);
  ...
  public void testProcess() throws InvalidParametersException,
    ...
    List<BsGene> allObjects = createGenomicregionGene(3);
    // Insert the first two rows to later update and delete.
    List<BsGene> existingObjects = 
      new CopyOnWriteArrayList<BsGene>();
    ...
    List<BsGene> insertObject = 
      new CopyOnWriteArrayList<BsGene>();
    insertObject.add(allObjects.get(2));
    
    // Put it all together.
    List<BsGene> objects = 
      new CopyOnWriteArrayList<BsGene>(existingObjects);
