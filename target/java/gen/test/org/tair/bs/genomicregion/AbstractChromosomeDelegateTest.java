...
/**
 * <p>
 * Test the ChromosomeDelegate class.
 * </p>
 * <p>
 ...
 */
public abstract class AbstractChromosomeDelegateTest {
  /** Define a class logger. */
  protected static Logger logger = Logger.getLogger(AbstractChromosomeDelegateTest.class);
  ...
  protected ChromosomeDelegate delegate = null;
  ...
  @Before
  public void setUp() {
    delegate = GenomicregionDelegateFactory.getChromosomeDelegate();
  ...
  protected void clearCaches() {
    // Primary class Chromosome cache
    IDaoManager manager = DaoManagerFactory.getManager(getSubsystem());
    if (manager != null) {
      manager.clearCache(org.tair.db.genomicregion.Chromosome.class.getName());
    }
  }

  /**
   * Create some number of new Chromosome objects and return them in a list.  
   ...
   * @throws DelegateException when a problem occurs during object creation
   */
  protected List<org.tair.bs.genomicregion.BsChromosome> createGenomicregionChromosome(int count) 
      throws DelegateException, InvalidParametersException {
    List<org.tair.bs.genomicregion.BsChromosome> objects = 
      new CopyOnWriteArrayList<org.tair.bs.genomicregion.BsChromosome>();
    ...
    for (int i = 0; i < count; i++) {
      ...
      // Create the object.
      BsChromosome object = null;
      try {
        object = delegate.createChromosome(tairObjectId, name, dateEntered, parentRegionTairObjectId, genome, sequence);
  ...
  public void testInsert() {
    // Create a new Chromosome object to perform the test.
    List<BsChromosome> objects = createGenomicregionChromosome(1);
    ...
    BsChromosome queriedObject = queryStoredObject(key);
    ...
    for (BsChromosome object : objects) {
      object.delete();
    }
   ...
   */
  protected BsChromosome queryStoredObject(com.poesys.db.pk.NaturalPrimaryKey key)
      throws DelegateException {
    // Clear the caches, then query the saved object and test it.
    clearCaches();

    BsChromosome queriedObject = delegate.getObject(key);
   ...
   *
   * @param object the BsChromosome object to update
   */
  protected void updateColumn(BsChromosome object) {
    try {
      object.setSequence("Chromosome.sequence updated");
   ...
   * @see #updateColumn
   */
  protected boolean isColumnUpdated(BsChromosome object) {
    boolean retVal = false;
    retVal = object.getSequence().equals("Chromosome.sequence updated");
    return retVal;
  }
   ...
   *
   * @param original the original, updated BsChromosome object
   * @param queried the queried BsChromosome object
   * @return true if the column is updated, false if not
   */
  protected boolean isColumnUpdated(BsChromosome original, BsChromosome queried) {
    boolean retVal = true;
    retVal = 
  ...
  public void testUpdateBatch() throws InvalidParametersException,
      DelegateException {
    List<BsChromosome> objects = createGenomicregionChromosome(2);
    delegate.insert(objects);
    // Allocate a map to hold the updated objects for later comparison.
    java.util.Map<com.poesys.db.pk.IPrimaryKey, BsChromosome> map = 
      new java.util.TreeMap<com.poesys.db.pk.IPrimaryKey, BsChromosome>();
    for (BsChromosome object : objects) {
      updateColumn(object);
    ...
    for (BsChromosome object : objects) {
      BsChromosome queriedObject = 
        queryStoredObject((com.poesys.db.pk.NaturalPrimaryKey)object.getPrimaryKey());
      BsChromosome originalObject = map.get(object.getPrimaryKey());
  ...
  public void testProcess() throws InvalidParametersException,
    ...
    List<BsChromosome> allObjects = createGenomicregionChromosome(3);
    // Insert the first two rows to later update and delete.
    List<BsChromosome> existingObjects = 
      new CopyOnWriteArrayList<BsChromosome>();
    ...
    List<BsChromosome> insertObject = 
      new CopyOnWriteArrayList<BsChromosome>();
    insertObject.add(allObjects.get(2));
    
    // Put it all together.
    List<BsChromosome> objects = 
      new CopyOnWriteArrayList<BsChromosome>(existingObjects);
    ...
    // Delete the inserted objects to clean up.
    // Mark all the objects for delete.
    for (BsChromosome object : allObjects) {
      object.delete();
    }
  ...
  public void testTruncateTable() {
    delegate.truncateTable("Chromosome");
    List<BsChromosome> objects = delegate.getAllObjects(100);
    assertTrue("Couldn't create list", objects != null);
